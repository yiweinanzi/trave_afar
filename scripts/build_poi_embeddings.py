...
"""
直接使用BGE-M3构建POI向量
不依赖FlagEmbedding库，直接使用transformers
"""
import os
import sys
import pandas as pd
...
def get_cache_file_path(poi_csv, output_dir, model_name):
    """生成缓存文件路径"""
    csv_hash = hashlib.md5(str(poi_csv).encode()).hexdigest()[:8]
    cache_dir = Path(output_dir) / "cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / f"poi_cache_{model_name}_{csv_hash}.pkl"
...
def main():
    ...
    output_emb_file = output_dir / f"poi_emb_{model_name}.npy"
    output_meta_file = output_dir / f"poi_meta_{model_name}.csv"
    cache_file = get_cache_file_path(poi_csv, output_dir, model_name)

    print("=" * 60)
    print("POI 向量构建工具 (BGE-M3)")
    print("=" * 60)
    print()

    # 检查已完成文件
    if output_emb_file.exists() and output_meta_file.exists():
        print("✓ 发现已完成的向量文件")
