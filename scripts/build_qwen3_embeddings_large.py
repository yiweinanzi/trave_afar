#!/usr/bin/env python3
"""
使用 Qwen3-Embedding-4B 构建 POI 向量 (大数据集版本)
使用 data/all/poi_expanded.csv (12万+ POI)
"""
import os
...
def get_cache_file_path(poi_csv, output_dir):
    ...
    return cache_dir / f"poi_cache_qwen3_large_{csv_hash}.pkl"
...
def build_poi_texts(df: pd.DataFrame) -> list:
    ...
    for _, row in df.iterrows():
        ...
        stay_hours = row.get('stay_min', 60) / 60
...
def main():
    ...
    batch_size = 128  # 更大的batch size提高吞吐
    use_gpu = True
    ...
    # 输出文件
    output_emb_file = output_dir / "poi_emb_large.npy"
    output_meta_file = output_dir / "poi_meta_large.csv"
    cache_file = get_cache_file_path(poi_csv, output_dir)

    print("=" * 60)
    print("POI 向量构建工具 (Qwen3-Embedding-4B) - 大数据集版本")
    print("=" * 60)
    print()
    ...
    if cache_file.exists():
        ...
        try:
            ...
            print(f"  已编码 {cached_count} 个POI，从断点继续...")
    ...
    if cache_data and 'embeddings' in cache_data:
        ...
        print(f"从缓存恢复，已编码 {start_idx} 个POI...")
    else:
        # 预分配数组
        print(f"初始化向量数组 ({total_pois} x {emb_dim})...")
